    ...
    if (message.id === getName(MessageEnum.RestAPIResponceMessage)) {
      ...
      if (apiRequestCallId === this.resetPasswordAPICallId) {
        ...
        if (responseJson && responseJson.errors) {
          this.setState({ showSpinner: false });
          if (responseJson.errors.full_phone_number && responseJson.errors.full_phone_number.length > 0) {
          ...
          if (responseJson.errors.length > 0) {
            this.setState({ showSpinner: false });
            //@ts-ignore
            window.notify([{ type: 'danger', message: responseJson.errors[0].message || responseJson.errors[0].email || 'Something Went Wrong!' }])
          }
        }
      }
    }
