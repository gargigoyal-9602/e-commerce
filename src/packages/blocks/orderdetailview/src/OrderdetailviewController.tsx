...
> {
  ...
  getTrackIdDetails = async () => {
    ...
    this.getTrackIdDetailsCallID = await this.apiCall({
      ...
      endPoint: `${configJSON.getTrackIdDetailsEndPoint}?id=${
        orderData.id
      }&track=order_item`,
    });
  };
  ...
  getLogisticTrackIdDetails = async () => {
    ...
    this.getLogisiticTrackIdDetailsCallID = await this.apiCall({
      contentType: configJSON.ApiContentType,
      method: configJSON.GetMethodType,
      endPoint: `${configJSON.shippingLogisiticAPIEndPoint}?order_item_id=${
        orderData.id
      }&track=order_item&order_id=${mainOrderData.id}`,
    });
  };
  ...
  getLogisticTrackIdDetailsSuccessCallBack = (res: any) => {
    ...
    orderDetails.product_name = this.props.navigation.state.params.orderData.product_name;
    orderDetails.quantity = this.props.navigation.state.params.orderData.quantity;
    orderDetails.unit_price_including_tax = this.props.navigation.state.params.orderData.unit_price_including_tax;
    orderDetails.is_review_present = this.props.navigation.state.params.orderData.is_review_present;
    orderDetails.is_item_cancelled = this.props.navigation.state.params.orderData.is_item_cancelled;
      ...
      setTimeout(() => {
        if (error !== "Shipment Not Found.") {
          ...
          let productDetails = {
            images: [
              {
                image: this.props.navigation.state.params.orderData
                  .product_image,
              },
            ],
          };
          ...
          orderDetails.product_name = this.props.navigation.state.params.orderData.product_name;
          orderDetails.quantity = this.props.navigation.state.params.orderData.quantity;
          orderDetails.unit_price = this.props.navigation.state.params.orderData.unit_price;
          orderDetails.is_review_present = this.props.navigation.state.params.orderData.is_review_present;
          orderDetails.is_item_cancelled = this.props.navigation.state.params.orderData.is_item_cancelled;
          orderDetails.order_number = this.props.navigation.state.params.mainOrderData.order_number;
          orderDetails.order_date = this.props.navigation.state.params.mainOrderData.order_date;
          trackingDetails.status = "Placed";
          trackingDetails.msg = "Your order is placed";
          trackingDetails.order_datetime = this.props.navigation.state.params.mainOrderData.order_date;
          trackingDetails.tracking_number = "";
          trackingDetails.order_date = this.props.navigation.state.params.mainOrderData.order_date;
          this.setState({
            orderDetails: orderDetails,
            productDetails: productDetails,
            trackingDetails: [trackingDetails],
  ...
  getVarientString = (properties: any) => {
    ...
    if (properties) {
      properties.map((property: any, index: any) => {
        varientString = `${varientString}${property.variant_name}: ${
