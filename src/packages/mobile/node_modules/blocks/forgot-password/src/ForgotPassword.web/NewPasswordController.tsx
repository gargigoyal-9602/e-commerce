...
> {
  ...
  setNewPass = async (values: any) => {
    console.log(values);
    const token = await StroageProvider.get('token');
    ...
    axios.put("https://ecommerceapp-39394-ruby.39394.dev.ap-southeast-1.aws.svc.builder.ai/account/accounts/reset_password", data)
      .then(res => {
        if (res.data) {
          // setInvalidPass('');
          this.setState({ invalidPass: "" })
          ...
          if (res?.data?.meta) {
            // @ts-ignore
            window.notify([{ message: res?.data?.meta?.message, type: "success" }])
          }
        }
      })
      .catch(error => {
