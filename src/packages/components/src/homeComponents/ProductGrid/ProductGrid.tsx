        ...
        return (
            <TouchableOpacity onPress={() => props.onPress(item)} style={styles.productGridStyle}>
                ...
                <View style={styles.imageMainContainer}>
                    <Image source={{ uri: item?.attributes?.images && item?.attributes?.images.data[0]?.attributes.url }} style={styles.BottalImage} />
                    ...
                    {item.attributes?.on_sale ? (<View style={styles.discountRow}>
                        <Text style={styles.price}>{themeJson.attributes.currency_type} {item?.attributes?.sale_price}</Text>
                        <Text style={styles.discountPrice}> {themeJson.attributes.currency_type} {item?.attributes?.price}</Text>
                    </View>) : (<Text style={[styles.price, { marginLeft: Scale(12) }]}>{themeJson.attributes.currency_type} {item?.attributes?.price}</Text>)}
                </View>
